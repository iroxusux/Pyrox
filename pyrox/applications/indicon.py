...
class BaseControllerValidator(plc.ControllerValidator):
    ...
    def _check_common_has_description(
        ...
        if not common_plc_object.description or common_plc_object.description == '':
            ...
            log(cls).log(LOG_LEVEL_SUCCESS,
                         f'{common_plc_object.__class__.__name__} {common_plc_object.name} description: {common_plc_object.description}')
    ...
    def _check_common_has_name(
        ...
        if not common_plc_object.name or common_plc_object.name == '':
            ...
            log(cls).log(LOG_LEVEL_SUCCESS, f'{common_plc_object.__class__.__name__} name: {common_plc_object.name}')
    ...
    def _check_datatype_member_has_valid_datatype(
        ...
        if not member.datatype or member.datatype == '':
            log(cls).log(LOG_LEVEL_FAILURE, f'Datatype member {member.name} in datatype {datatype.name} has no datatype!')
            ...
            log(cls).log(LOG_LEVEL_SUCCESS, f'Datatype member {member.name} in datatype {datatype.name} datatype: {member.datatype}'
                         )
    ...
    def _check_routine_has_jsr(
        ...
        if routine.name == program.main_routine_name:
            log(cls).log(LOG_LEVEL_SUCCESS, f'Routine {routine.name} in program {program.name} is the main routine.')
            return
        if not program.check_routine_has_jsr(routine):
            ...
            log(cls).log(LOG_LEVEL_SUCCESS, f'Routine {routine.name} in program {program.name} has JSR calls to it.')
    ...
    def validate_datatype_member(
        ...
        log(cls).info(f'Validating datatype member {member.name} in datatype {datatype.name}...')
        cls._check_common_has_name(controller, member)
        cls._check_common_has_description(controller, member)
    ...
    def validate_rung(
        ...
        log(cls).info(f'Validating rung {rung.number} in routine {routine.name} in program {program.name}...')
        ...
        if not rung.number or rung.number == '':
            ...
            log(cls).log(LOG_LEVEL_SUCCESS, f'Rung number: {rung.number}')
        ...
        if not rung.text or rung.text == '':
            ...
            log(cls).log(LOG_LEVEL_SUCCESS, f'Rung {rung.number} text: {rung.text}')
    ...
    def validate_tag(
        ...
        if not tag.datatype or tag.datatype == '':
            ...
            log(cls).log(LOG_LEVEL_SUCCESS, f'Tag {tag.name} datatype: {tag.datatype}')

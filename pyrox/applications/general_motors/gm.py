...
class GmController(GmPlcObject, Controller):
    ...
    def generate_emulation_logic(self):
        """Generate the emulation logic for the controller
        """
        if not self.mcp_program:
            self.logger.error('No MCP program found in the controller.')
            return

        if not self.mcp_program.main_routine:
            self.logger.error('No main routine found in the MCP program.')
            return

        if not self.safety_common_program:
            self.logger.error('No safety common program found in the controller.')
            return

        if not self.safety_common_program.main_routine:
            self.logger.error('No main routine found in the safety common program.')
            return

        schema = ControllerModificationSchema(source=None,
                                              destination=self)

        emu_routine: GmRoutine = GmRoutine(controller=self)
        emu_routine.name = 'aaa_Emulation'
        emu_routine.description = 'Emulation routine for GM controller. This routine is auto-generated by Indicon LLC. Do not modify.'
        emu_routine.clear_rungs()
        schema.add_routine_import(program_name=self.mcp_program.name,
                                  routine=emu_routine)
        schema.add_rung_import(program_name=self.mcp_program.name,
        ...
        schema.add_program_tag_import(program_name=self.mcp_program.name,
        ...
        s_emu_routine.description = 'Emulation routine for GM controller in safety context. This routine is auto-generated by Indicon LLC. Do not modify.'  # noqa: E501
        s_emu_routine.clear_rungs()
        schema.add_routine_import(program_name=self.safety_common_program.name,
                                  routine=s_emu_routine)
        schema.add_rung_import(program_name=self.safety_common_program.name,
        ...
        emu_routine.add_rung(Rung(controller=self, text='MOV(0,Uninhibit)MOV(4,Inhibit);',

...
check_python_installed() {
    log_and_echo "Checking for Python installation..."
    ...
    for cmd in "${python_commands[@]}"; do
        if command -v "$cmd" &> /dev/null; then
            log_and_echo "Found Python command: $cmd"
            # Test if it actually works
            if "$cmd" --version &> /dev/null; then
                local version=$("$cmd" --version 2>&1)
                log_and_echo "✓ Python detected: $version (command: $cmd)"
                found_python="$cmd"
                break
            else
                log_and_echo "Warning: $cmd command found but not working"
            fi
        fi
    done
    
    if [ -z "$found_python" ]; then
        log_error "Python is not installed or not working properly"
        log_and_echo "PATH contents:"
        echo "$PATH" | tr ':' '\n' | while IFS= read -r path_entry; do
            log_and_echo "  $path_entry"
        done
        return 1
    fi
...
check_python_version_installed() {
    ...
    local required_version="${required_version_major}.${required_version_minor}"
...
safe_ensure_python_installed() {
    ...
    log_and_execute "Ensuring Python ${required_major}.${required_minor}+ is installed" ensure_python_installed "$required_major" "$required_minor"
    if [ $? -ne 0 ]; then
...
ensure_python_installed() {
    ...
    prompt_and_install_python "$required_major" "$required_minor"
    if [ $? -ne 0 ]; then
        return 1
    fi
    
    # Verify the installation
    verify_python_installation "$required_major" "$required_minor"
    return $?
}
...
prompt_and_install_python() {
    local required_major="$1"
    local required_minor="$2"
    local required_version="${required_major}.${required_minor}"
    
    # Validate parameters
    if [ -z "$required_major" ] || [ -z "$required_minor" ]; then
        log_and_echo "Error: prompt_and_install_python requires major and minor version numbers"
        return 1
    fi
    ...
    if check_python_version_installed "$required_major" "$required_minor" >/dev/null 2>&1; then
        log_and_echo "✓ Required Python version is already installed."
        return 0
    fi
    ...
    case "$choice" in 
        [yY]|[yY][eE][sS])
            ...
            if install_python_automatically "$required_major" "$required_minor"; then
...
install_python_automatically() {
    local required_major="$1"
    local required_minor="$2"
    local python_version="${required_major}.${required_minor}"
...
# Parameter 1: Python version (e.g., 3.13)
update_windows_path_for_python() {
    ...
    local version_nodot="${version//./}"  # e.g., 3.13 -> 313
    local python_dirs=(
...
install_python_windows() {
    ...
    # Get the latest patch version (e.g., 3.13.0)
    ...
    if [ -z "$latest_patch_version" ]; then
        latest_patch_version="${version}.0"  # Fallback
    fi
...
verify_python_installation() {
    ...
    local python_paths=(
        ...
        "/usr/local/bin/python${required_major}.${required_minor}"
        "/usr/bin/python${required_major}.${required_minor}"
    ...
    if [[ "$OSTYPE" == "msys" || "$OSTYPE" == "win32" || "$OSTYPE" == "cygwin" ]]; then
        ...
        for python_path in "${windows_paths[@]}"; do
            if [ -f "$python_path" ]; then
                ...
                if "$python_path" -c "import sys; exit(0 if sys.version_info >= ($required_major, $required_minor) else 1)" 2>/dev/null; then
...
EOF
    ...
    if command -v python &> /dev/null; then
        if python -c "import sys; exit(0 if sys.version_info >= ($required_major, $required_minor) else 1)" 2>/dev/null; then
